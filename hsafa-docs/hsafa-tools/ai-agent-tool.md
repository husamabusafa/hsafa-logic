...
```json
{
  ...
  "messages": [
    // Array of previous messages + new user message + new assistant message
  ],
  "isAbort": false,
  "isDisconnect": false,
  "isError": false,
...
### Response Structure
- **message**: The final assistant message generated by the agent. Includes detailed `parts` for every step (tools, text, etc).
- **messages**: The full conversation history including the new messages.
- **parts**: Detailed execution trace.
  - `step-start`: Indicates start of a reasoning step.
  - `tool-{name}`: Tool execution. Includes input, output, and state.
