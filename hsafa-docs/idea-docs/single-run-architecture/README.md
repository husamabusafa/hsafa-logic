...
- **Composite messages:** A run produces one message per space. `sendSpaceMessage` adds text parts, client tools add UI parts, visible server tools add tool-card parts — all in one cohesive message bubble.
...
5. **[Composite Messages & Tool Visibility](./05-space-ui.md)** — Composite message model (parts accumulation), tool visibility levels (hidden/minimal/full), cross-space UI pattern
...
### Phase 3: Composite Messages + Tool Visibility
Replace per-tool-call message creation with composite message model (one message per run per space, parts accumulate). Add `visibility` field to tool config. Auto-inject `targetSpaceId`/`targetSpaceIds` into tool schemas. Update `stream-processor.ts` to route parts to correct spaces.

### Phase 4: SDK Updates
Update react-sdk (`useHsafaRuntime`) to handle composite messages, text-delta from tool-input interception, and `targetSpaceId` routing. Update node-sdk with service trigger support. Update ui-sdk to render composite message parts (text + tool cards + custom UI inline).
