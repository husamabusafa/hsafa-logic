...
The update: instead of thinking only in “one agent ↔ one chat”, the platform’s native model becomes **many Entities collaborating inside shared contexts** (Nexuses).

---

# 🕸️ Agentic Network Model (Nexuses + Entities)
...
- Nexuses can be public (many participants) or private (small, scoped contexts like “Manager Nexus”).
...
# 🔁 Example scenario: Leave Request (Cross-Nexus)
...
The Agent can mark a tool call as visible to **all Entities** in the Nexus. This allows:
...
# 🧰 Main Tools every Agent Entity should have
