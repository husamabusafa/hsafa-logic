...
export interface UseHsafaRuntimeOptions {
  ...
  onSwitchSpace?: (smartSpaceId: string) => void;
  onNewSpace?: () => void;
}
...
export function useHsafaRuntime(options: UseHsafaRuntimeOptions): UseHsafaRuntimeReturn {
  ...
  useEffect(() => {
    ...
    // Restore active agents + pending tool calls from SSE connected event
    ...
    // run.waiting_tool → deprecated in v3, kept for v2 compat
    ...
    // space.message → persisted message arrived
  ...
  const messages = useMemo<ThreadMessageLike[]>(() => {
    ...
    // Visible tool calls as assistant messages
  ...
  // ── Send message (simple string) ──
  const send = useCallback(
    async (text: string) => {
      if (!smartSpaceId) throw new Error('No SmartSpace selected');
      const { message: sent } = await client.messages.send(smartSpaceId, { content: text, entityId });
  ...
  return { messages, isRunning, activeAgents, onNew, send, membersById };
}
