...
type HighlightFn = (line: string) => React.ReactNode;
...
export const CodeTerminal = forwardRef<CodeTerminalHandle, CodeTerminalProps>(
  function CodeTerminal(
    ...
    // Read-only mode — content div drives line numbers
    const syncReadOnlyScroll = useCallback(() => {
      const el = contentRef.current;
      if (!el || !lineNumbersRef.current) return;
    ...
    useEffect(() => {
      if (editable) {
        ...
        const el = contentRef.current;
        if (!el) return;
        el.addEventListener("scroll", syncReadOnlyScroll);
        return () => el.removeEventListener("scroll", syncReadOnlyScroll);
      }
    }, [editable, syncScroll, syncReadOnlyScroll]);
