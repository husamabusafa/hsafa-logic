...
- [x] `routes/global-skills.ts` — Skill registry + tool registration + SSE stream
- [x] `routes/global-actions.ts` — Action result submission

### 2f. index.ts — ✅ DONE
- [x] Wire all v7 routes (haseefs, events, skills, actions, runs, memory, dashboard)
...
| **Memory search** | Basic key lookup | Keyword search (pgvector planned, currently disabled) |
| **Process manager** | Long-running processes per haseef | No processes — trigger-based invocation |
| **Tool routing** | Per-haseef + global scopes | Global skills only |
| **Reflection** | Consciousness compaction | Post-run episodic memory extraction |

---

## v5 Artifacts — ALL REMOVED
All v5 dead code has been deleted from `core/src/`. The current tree contains only v7 files.

Also removed:
- `hsafa-core/sdks/service-node/` — Old v5 SDK (replaced by `sdks/hsafa-sdk`)
- `/api/scopes` legacy route alias
- `req.body.scope` legacy field fallback in `events.ts`
- All "scope" terminology — unified to "skill" everywhere
