...
The update: instead of thinking only in “one agent ↔ one chat”, the platform’s native model becomes **many Entities collaborating inside shared contexts** (SmartSpaces).

---

# 🕸️ Agentic Network Model (SmartSpaces + Entities)
...
- SmartSpaces can be public (many participants) or private (small, scoped contexts like “Manager SmartSpace”).
...
External services (Jira, Slack, IoT devices, payment gateways, cron jobs, Node.js backends) are **NOT entities**. They are **services** — they trigger agents via API (`POST /api/agents/{agentId}/trigger`) and submit tool results, but have no space membership or identity in the system.
...
- A Client is **not** an Entity — it is a delivery channel
...
A **Run** is a general-purpose execution of **one Agent Entity**. Runs are **not tied to any space** — the agent interacts with spaces through tools (`sendSpaceMessage`, `readSpaceMessages`).
...
* **`space_message`** — A human (or agent via `mention`) posted a message in a space
* **`plan`** — The agent's own scheduled plan triggered it
* **`service`** — An external service triggered the agent directly via API (`POST /api/agents/{agentId}/trigger`)

## Scheduling and long-term behavior

Each Agent Entity maintains:

* **Plan**: schedules future executions (cron-like). Example: “send a reminder at 6 PM”.
...
# 🔁 Example scenario: Leave Request (Cross-SmartSpace)
...
3. The Agent calls `sendSpaceMessage(managerSpace, "Sarah is requesting 2 days PTO next week. Approve?", wait: { for: [{ type: "human" }] })` — the message streams into the Manager SmartSpace and the tool blocks until the manager replies.
4. The Manager sees the message and replies "Approved."
5. The Agent's `wait` resolves with the manager's reply.
6. The Agent calls `sendSpaceMessage(employeeSpace, "Your leave request was approved!")` — streams back to the employee.

All of this happens in **one general-purpose run**. The agent uses `sendSpaceMessage` with `wait` for cross-space request-response.
...
# 🧰 Display Tools and Space Routing

## `displayTool` + Auto-Injected `targetSpaceId` & `mention`
...
When `displayTool: true`, the gateway auto-injects optional `targetSpaceId` and `mention` into the tool schema seen by the model.
...
- If the AI also provides `mention` (an agent entity ID), the mentioned agent is triggered after the tool call message is posted to the target space — same behavior as `sendSpaceMessage` with `mention`.
- If the AI omits `targetSpaceId`, the tool executes normally and remains internal to the run stream (`mention` is ignored without a target space).
- The gateway strips both `targetSpaceId` and `mention` before calling `execute()`.

### Routing Behavior

| `displayTool` | `targetSpaceId` in call | `mention` in call | Result |
|---|---|---|---|
| `false` / omitted | any | any | Internal execution only (no space message) |
| `true` | omitted | any | Internal execution only (silent tool call, `mention` ignored) |
| `true` | set | omitted | Tool call part in target space |
| `true` | set | set | Tool call part in target space + mentioned agent triggered |
...
# 🎨 Tool UI Configuration (Client-Side)

> **Note:** The gateway decides **whether** a tool call is visible in a SmartSpace (`displayTool` + `targetSpaceId`) and **whether** to trigger an agent (`mention`). The client SDK decides **how** that routed tool call is rendered.

## Architecture

**Gateway routing responsibilities:**
- Inject `targetSpaceId` and `mention` only for tools with `displayTool: true`
...
- `displayTool` controls routing capability (`targetSpaceId` + `mention` injection)
- `targetSpaceId` in each call controls where (or whether) the tool appears
- `mention` in each call controls whether an agent is triggered after the tool message is posted
- `display.customUI` controls the component used for rendering

---

# 🧩 Custom Tool UI Components (TODO)
...
- **`@hsafa/react`** — Hooks-only SDK for data fetching (`useMessages`, `useHsafaClient`, etc.)
- **`@assistant-ui/react`** — UI primitives (`MessagePrimitive`, `ThreadPrimitive`, etc.)
...
function ApprovalToolUI({ args, result, status, addResult }: ToolUIProps) {
  // Already completed
  if (result) {
    return <div>{result.approved ? "✅ Approved" : "❌ Rejected"}</div>;
  }
...
`displayTool` (with `targetSpaceId` + `mention`) and `display.customUI` are complementary:
...
- `displayTool: true` enables optional `targetSpaceId` routing and `mention` triggering
- A call with `targetSpaceId` renders a tool part in that SmartSpace
- A call with `targetSpaceId` + `mention` renders a tool part AND triggers the mentioned agent
- A call without `targetSpaceId` stays internal (no SmartSpace tool part, `mention` ignored)
...
# 🧰 Prebuilt tools every Agent Entity should have
...
6. **Space communication tools**
   ...
   * `sendSpaceMessage` — send a message to any space (with optional `mention` to trigger another agent, optional `wait` to block for a reply)
   * `readSpaceMessages` — read recent messages from any space
   * `delegateToAgent` — (admin only) silently hand off to another agent
   * `getMyRuns` — see own concurrent active runs
