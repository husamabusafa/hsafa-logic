...
export function createSetPlansTool(ctx: HaseefProcessContext) {
  return tool({
    description:
    ...
    execute: async ({ plans }) => {
      ...
      for (const plan of plans) {
        ...
        if (plan.id) {
          ...
          await enqueuePlan({ id: plan.id, entityId: ctx.haseefEntityId, cron: plan.cron ?? null, nextRunAt, isRecurring });
        } else {
          ...
          // Enqueue new plan
          await enqueuePlan({ id: created.id, entityId: ctx.haseefEntityId, cron: plan.cron ?? null, nextRunAt, isRecurring });
        }
        count++;
      }
